...
const PostForm: React.FC<PostFormProps> = ({ onPostSuccess }) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post('/api/posts', { text: postText });
      ...
      setError(error.response?.data?.message || 'An error occurred while posting your comment.');
    }
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex flex-1">
        <Sidebar />
        <MainContent postText={postText} handleInputChange={handleInputChange} handleSubmit={handleSubmit} isSubmitting={isSubmitting} error={error} />
      </div>
      <Footer />
    </div>
  );
...
const MainContent: React.FC<MainContentProps> = ({ postText, handleInputChange, handleSubmit, isSubmitting, error }) => {
  return (
