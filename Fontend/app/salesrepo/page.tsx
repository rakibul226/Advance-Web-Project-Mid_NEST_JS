...
const SalesReportPage: React.FC = () => {
  ...
  const fetchSales = async () => {
    try {
      const response = await axios.get<Sale[]>('http://localhost:3000/sales'); 
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex flex-1">
        <Sidebar />
        <MainContent sales={sales} headers={headers} />
      </div>
      <Footer />
    </div>
  );
...
const MainContent: React.FC<MainContentProps> = ({ sales, headers }) => {
  return (
